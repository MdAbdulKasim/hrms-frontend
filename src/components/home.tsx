...
const ProfileCard = () => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6 flex flex-col items-center text-center border border-gray-100">
      ...
      <div className={`bg-gray-100 px-4 py-2 rounded-md mt-3 font-mono font-medium tracking-wider ${isCheckedIn ? 'text-gray-900' : 'text-gray-600'}`}>
        {formatTime(seconds)}
      </div>
      <button 
...
const ReporteesCard = () => {
  return (
    <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100 flex flex-col h-full">
      ...
      <div className="flex-1 space-y-5">
        {reportees.map((person) => (
          <div key={person.id} className="flex items-start gap-3">
            ...
            <div>
              <p className="text-xs text-gray-500 font-medium">{person.roleId} - {person.name}</p>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-gray-800 font-medium">Good Afternoon <span className="text-gray-500 font-normal">farhan21ps13716</span></h3>
          ...
          <div className="grid grid-cols-7 gap-2 relative z-10">
            {schedule.map((item, index) => (
              <div key={index} className="flex flex-col items-center">
                <div className={`w-2.5 h-2.5 rounded-full mb-3 ${item.isToday ? 'bg-blue-500 ring-4 ring-blue-100' : 'bg-gray-300'}`}></div>
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 flex items-center gap-3">
        <div className="p-1.5 bg-yellow-50 rounded-full text-yellow-500">
           <AlertCircle size={18} />
        </div>
        <p className="text-gray-700 text-sm">You are yet to submit your time logs today!</p>
      </div>
    </div>
  );
};
...
export default function Dashboard() {
  return (
      ...
      <div className="min-h-screen bg-[#f3f4f6] p-4 md:p-8 font-sans">
        ...
        {/* Main Content Area - No more Tabs */}
